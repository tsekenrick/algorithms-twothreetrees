...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int entryCount = Integer.parseInt(br.readLine());
        ...
        for(int i = 0; i < entryCount; i++) {
            ...
            twothree.insert(data[0], Integer.parseInt(data[1]), tree);
        }
        ...
        for(int i=0; i<queryCount; i++){
            String[] range = queries.get(i).split("\\s");
            
            //if-else accounts for query being in reverse order
            if(range[0].compareTo(range[1]) <= 0){
                twothree.printRange(tree.root, range[0], range[1], tree.height, output);
            }
            else{
                twothree.printRange(tree.root, range[1], range[0], tree.height, output);
            }
        }
        output.flush();
    }
...
class twothree {
    ...
    /**
     * Print all leaves in a tree of given root
     * @param node root of tree
     * @param h height of tree
     */
    static void printAll(Node node, int h, BufferedWriter output) throws IOException{
        //base case
        if(h == 0 && node instanceof LeafNode) {
            LeafNode p = (LeafNode)node;
            output.write(p.guide + " " + p.value + "\n");
        }
        
        else {
            if(node instanceof InternalNode){
                InternalNode p = (InternalNode)node;
                printAll(p.child0, h-1, output);
                printAll(p.child1, h-1, output);
                if(p.child2 != null){
                    printAll(p.child2, h-1, output);
                }
            }
    ...
    /**
     * Prints all leaves for tree rooted at a given node whose key is >=x
     * @param node root of tree
     * @param x key for >= comparison
     * @param h height of tree
     */
    static void printGE(Node node, String x, int h, BufferedWriter output) throws IOException{
        //base case
        if(h == 0 && node instanceof LeafNode) {
            LeafNode p = (LeafNode)node;
            ...
            if (p.child0.guide.compareTo(x) >= 0){
                ...
                printGE(p.child1, x, h-1, output);
                if(p.child2 != null) printAll(p.child2, h-1, output);
            }
            else{
                printGE(p.child2, x, h-1, output);
            }
        }
    }
    /**
     * Print all leaves for tree rooted at given node whose key is <= x
     * @param node root of tree
     * @param x key for <= comparison
     * @param h height of tree
     */
    static void printLE(Node node, String x, int h, BufferedWriter output) throws IOException{
        //base case
        if(h == 0 && node instanceof LeafNode) {
            LeafNode p = (LeafNode)node;
            if(p.guide.compareTo(x) <= 0){
            ...
            InternalNode p = (InternalNode) node;
            if (p.child0.guide.compareTo(x) >= 0){
                printLE(p.child0, x, h-1, output);
            }
            else if (p.child2 == null || p.child1.guide.compareTo(x) >= 0){
                printAll(p.child0, h-1, output);
                printLE(p.child1, x, h-1, output);
            }
            else{
                printAll(p.child0, h-1, output);
                printAll(p.child1, h-1, output);
                printLE(p.child2, x, h-1, output);
            }
        }
    }
    
    static void printRange(Node node, String x, String y, int h, BufferedWriter output) throws IOException{
        if(h==0 && node instanceof LeafNode){
            LeafNode p = (LeafNode)node;
            ...
            if(p.child0.guide.compareTo(x) >= 0){
                printRange(p.child0, x, y, h-1, output);
                printLE(p.child1, y, h-1, output);
                if(p.child2 != null) printLE(p.child2, y, h-1, output);    
            }
            //if true, lower bound within child1
            else if (p.child2 == null || p.child1.guide.compareTo(x) >= 0){
                printRange(p.child1, x, y, h-1, output);
                if(p.child2 != null) printLE(p.child2, y, h-1, output);
            }
            else {
                printRange(p.child2, x, y, h-1, output);
            }
        }
    }
